...
#        print(" hf =  ",hf, ' k = ;' , k, ' ans = ' ,ans , ' i = ' , i)
        if abs(hf - h) < 0.001:
            if k == 5:
                ans = ans + Simp38(h, y[i-5], y[i-4], y[i-3], y[i-2])
                PlotSimp38(x[i-5], x[i-4], x[i-3], x[i-2],
                           y[i-5], y[i-4], y[i-3], y[i-2])
                TickSimp38(x[i-5], x[i-4])
                TickSimp38(x[i-4], x[i-3])
            ...
            if k == 1:        # trap
                ...
                ans = ans + Simp13(h, y[i-4], y[i-3], y[i-2])
                PlotSimp13(x[i-4], x[i-3], x[i-2], y[i-4], y[i-3], y[i-2])
                TickSimp13(x[i-4], x[i-3])
...
#    print(points)
    ...
    print('Integral value: %.5f'%( Area(points)))
    SetLegend()
