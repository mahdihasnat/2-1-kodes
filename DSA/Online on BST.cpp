...
//Node * input_tree(Node * parent,bool isleft)
//{
//    cout<<" enter "
//    if(parent)
//    {
//        cout<<"Enter "<< (isleft?" left ": " right ")<<" child of "<<parent->value<<" \n";
//    }
//    else
...
//            DBG(mid);
    ...
    int store_inorder(Node<T> * head,int *a,int n) /// returns size
    {
        if(head)
        {
            n = store_inorder(head->left,a,n);
            a[n++] = head->value;
            n= store_inorder(head->right,a,n);
        }
        return n;
    }
    void free_memory(Node<T> * head)
    {
        if(head)
        {
...
int main()
{
    ...
    do
    {
        ...
        {
            binTree.make_balanced();
            cout<<" now bstree is balanced\n";
        }
    }
    while(choice !=7);
}
